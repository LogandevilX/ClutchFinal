...
export async function fetchEstadoPartido(partidoId) {
  const response = await fetchJson(`${PARTIDOS_URL}/${partidoId}/estado`);

  if (!response.ok) {
    throw new Error('No se pudo obtener el estado del partido.');
  }

  return response.data;
}

export async function fetchJugadorActas(jugadorId) {
...
export async function postEventoPartido({
  partidoId,
  equipoId,
  jugadorId,
  accion,
  ...
  const body = {
    ...
    acierto: typeof acierto === 'boolean' ? String(acierto) : acierto || null,
  };
  ...
  if (!response.ok) {
    throw new Error('No se pudo registrar el evento del partido.');
  }
...
export async function finishPeriod(partidoId) {
  ...
  if (!response.ok) {
    throw new Error('No se pudo finalizar el periodo actual.');
  }
...
export async function startPeriod({ partidoId, periodo, minuto = 0, titulares = [] }) {
  ...
  if (!response.ok) {
    throw new Error('No se pudo iniciar el periodo indicado.');
  }

  return response.data;
}

export async function closeMatch(partidoId) {
...
export async function fetchInitialMatchSetup(selectedMatch) {
  ...
  const [equipoLocal, equipoVisitante, inscripcionesLocal, inscripcionesVisitante, estadoPartido] = await Promise.all([
    ...
    fetchEstadoPartido(selectedMatchId).catch(() => null),
  ]);
